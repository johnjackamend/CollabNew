...
- (instancetype)init
{
    ...
    if (self)
    {
        [self setupViewWindowWidth:self.view.frame.size.width - 60.f];
    }
    return self;
}
...
{
    self = [self initWithWindowWidth:self.view.frame.size.width - 60.f];
    if(self)
    {
        [self setupNewWindow];
    }
...
- (SCLAlertViewResponder *)showTitle:(UIViewController *)vc image:(UIImage *)image color:(UIColor *)color title:(NSString *)title subTitle:(NSString *)subTitle duration:(NSTimeInterval)duration completeText:(NSString *)completeText style:(SCLAlertViewStyle)style
{
    ...
    {
        case Success:
            viewColor = UIColorFromHEX(0xE5001C);//0x22B573
            iconImage = SCLAlertViewStyleKit.imageOfCheckmark;
            break;
            
        case Error:
            viewColor = UIColorFromHEX(0xE5001C);//C1272D
        ...
        case Warning:
            viewColor = UIColorFromHEX(0xE5001C);
            iconImage = SCLAlertViewStyleKit.imageOfWarning;
            break;
            
        case Info:
            viewColor = UIColorFromHEX(0x26A64F);//0x2866BF
            iconImage = SCLAlertViewStyleKit.imageOfInfo;
            break;
            
        case Edit:
            viewColor = UIColorFromHEX(0x26A64F);//A429F
    ...
    else
    {
        ...
        self.circleIconImageView.image = [UIImage imageNamed:@"splashLogo"];
    }
